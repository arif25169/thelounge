...
Chan.prototype.writeUserLog = function(client, msg) {
	...
	for (const messageStorage of client.messageStorage) {
		messageStorage.index(target.network.uuid, this.name, msg);
	}
};

Chan.prototype.loadMessages = function(client, network) {
	if (client.messageStorage.length === 0 || !this.isLoggable()) {
		return;
	}

	// TODO: Something more configurable, right now it will
	// try to load messages from the first configured log storage
	client.messageStorage[0]
